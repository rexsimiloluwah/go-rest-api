...
import (
	...
	u "github.com/rexsimiloluwah/go-rest-api/utils"
)
...
func GetPostById(post_id uint) map[string]interface{} {
	post := &Post{}
	var response map[string]interface{}
	err := Db().Table("posts").Where("id = ?", post_id).First(post)
	if err != nil {
...
func GetUserPosts(user_id uint) map[string]interface{} {
	...
	var response map[string]interface{}
	err := Db().Table("posts").Where("user_id = ?", user_id).Find(&userPosts)
	fmt.Println(len(userPosts))
	if err != nil {
		if err.Error == gorm.ErrRecordNotFound || len(userPosts) == 0 {
...
func UpdatePost(post_id uint, fields map[string]interface{}) map[string]interface{} {
	var response map[string]interface{}
	err := Db().First(&Post{}, post_id)
	if err.Error == gorm.ErrRecordNotFound {
		response = u.Message(false, "Post does not exist.")
		return response
	}

	err = Db().Table("posts").Where("id = ?", post_id).Updates(fields)
	if err.Error != nil {
